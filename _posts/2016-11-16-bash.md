...
This is a mixture of a little introduction to some trivias of bash language and introduction of some frequently used bash commands. Basically every time I encounter a useful bash commands or when I learn something new about a command, I write them down for future reference.

Topics:
- [Others](#others)

## Show recently modified files
This along with many other ways of displaying files could be accomplished using function `find`. Some examples  are as follows:

- Find files modified within a day:	
	- `find . -mtime -1`
	- For over a day, replace `-` in front of 1 with `+`
...
Say my previous command is `vim project.txt`. Now I want to `open` this file instead of using `vim`. Then I can simply input: `$ !vim:s/vim/open`, where `$` is the shell prompt. Basically this is performing `sed` on whatever the results are from `!vim`.

- **Redirection**    
Bash shell has 3 basic streams: `input(0)`, `output(1)`, and `error(2)`. We can use `#number>` to redirect them to somewhere else, eg:
	- Input redirection: 
		1.  `<` or `0<`
		2.  `command << EOF`, and then manually input argument file, using `EOF` to end inputting (or use `ctrl + D`). `<<` is `here document` symbol.
		3.  `command <<< string` : it's `here string` symbol. Can input a one row string argument.   
